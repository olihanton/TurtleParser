...
void drawline(Turtleposition *t, SDL_Simplewin *sw){
   Neill_SDL_SetDrawColour(sw, t->red, t->blue, t->green);
...
void subseqcolour(Turtleposition *t, Program *p){
   ...
   else{ERROR("Token 'COLOUR' must be followed by a defined colour");}
   executecolour(t, p);
}
...
void executesmooth(Turtleposition *t, Program *p){
   if(strsame(p->wds[p->cw], "ON")){
      ...
      Neill_SDL_RenderDrawLine(sw->renderer, t->prevprev[0], t->prevprev[1], t->prev[0], t->prev[1])
      Neill_SDL_RenderDrawLine(sw->renderer, t->prev[0], t->prev[1], t->current[0], t->current[1])
      Neill_SDL_SetDrawColour(sw, t->red, t->green, t->blue);
      // need to take into account if nodraw has been set or not...
      // here we would also need to include the actual smooth function itself.
   }
}*/
