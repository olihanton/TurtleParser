/* Code which takes an input set of instructions, parses them and then interprets the output they give and */


/*FIX STRING LENGTH AND ALSO FIX CHECK INTEGER - THEN TRANSFER THESE TO THE EXTENSION*/
...
#define ARBDELAY 200       /*delay in milliseconds between instuction prints*/
...
void checkpolish(Program *p, Stack *s){ 
   ...
   if(!op(p->wds[p->cw]) && numorcaps(p->wds[p->cw])){    
      if(caps(p->wds[p->cw])){
         sscanf(p->wds[p->cw], "%c", &num);
